...
- `K_growth` ranges from 0.25 (Euphausiids) to 0.55 (Flagellates, Ciliates), reflecting different life-history strategies
- **Zooplankton** have `repro_on = 0` (reproduction is not explicitly modelled). Most zooplankton groups have `R_frac > 0` (0.15–0.20), representing real reproductive costs. Below maturation size, R_frac energy is redirected to somatic growth; above maturation size, it is lost from the modelled system. Zooplankton recruitment is instead maintained via boundary conditions. Flagellates and Ciliates have `R_frac = 0` because they reproduce by binary fission rather than allocating a separable fraction of production to offspring.
...
Note that energy budget fractions are group-specific. For all groups with R_frac > 0, the allocation depends on maturity: **immature individuals** (below Wmat) redirect R_frac to somatic growth (effective growth = K_growth + R_frac), while **mature individuals** allocate R_frac either to explicit reproduction (fish) or lose it from the system (zooplankton). The transition is governed by a size-based maturity ogive.
...
- **Scenario A (default):** Full energy budget for all groups. Most zooplankton groups have `R_frac > 0` (0.15–0.20), representing reproductive energy that is redirected to growth below maturation size and lost above maturation size. Flagellates and Ciliates have `R_frac = 0` (binary fission). This is biologically realistic but means not all assimilated energy contributes to modelled growth for mature zooplankton.
...
```{r fig.height=6}
# Growth rates for zooplankton (K_growth ranges from 0.25 to 0.55 across groups)
...
1. **Energy Budget Components**: The model explicitly tracks non-nutritive fraction (defecation + water/structural losses), metabolism, growth, and reproduction fractions. Energy budget parameters (`f_M`, `K_growth`, `R_frac`) are group-specific, with K_growth ranging from 0.25 (Euphausiids) to 0.55 (Flagellates, Ciliates). For zooplankton with R_frac > 0, immature individuals redirect R_frac to growth while mature individuals lose it from the system; for fish, R_frac drives explicit SSB-based recruitment. Flagellates and Ciliates have R_frac = 0 (binary fission).
...
4. **Fish Reproduction**: Fish groups allocate a group-specific fraction of assimilated energy (R_frac) to reproduction, scaled by a maturity ogive. Immature fish and zooplankton redirect R_frac to somatic growth.
...
|--------|-------------|------|
| K_growth | 0.25–0.55 (group-specific) | 0.30 (all fish groups) |
| R_frac | 0.00–0.20 (group-specific; 0.00 for Flagellates/Ciliates, 0.15–0.20 for others) | 0.20 (= 1 - f_M - K_growth; drives explicit reproduction) |
