...
Groups_repro1$repro_eff[Groups_repro1$Type == "Fish"] <- .01
...
```{r}
env_repro1 <- createInputParams(time = seq(0, 800, by = 0.1),
                                sst = 15,
                                chl = 1)

mdl_repro1 <- zoomss_model(input_params = env_repro1, Groups = Groups_repro1, isave = 5)
```
...
p_repro1_biomass <- plotTimeSeries(mdl_repro1, by = "biomass", transform = "log10") +
  ggtitle("Biomass (repro_eff = 1)") +
  theme(legend.position = "none")

p_repro1_fill <- plotTimeSeries(mdl_repro1, by = "biomass", type = "fill") +
  ggtitle("Proportional biomass (repro_eff = 1)")
