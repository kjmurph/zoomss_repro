# ============================================================================
# PRE-COMPUTE ALL ZOOMSS MODELS FOR DUAL-PATHWAY BRANCH TESTING
# ============================================================================
#
...
#
# Usage:
#   source("vignettes/run_gradient_models.R")
#
...
n_years    <- 300   # Total simulation years
dt_step    <- 0.1   # Time step (years)
isave_freq <- 2     # Save every 2nd time step
...
cat("==================================================\n")
cat("ZooMSS Dual-Pathway Gradient Model Pre-Computation\n")
cat("==================================================\n")
cat("Workers:   ", n_cores, "\n")
cat("Years:     ", n_years, "\n")
cat("dt:        ", dt_step, "\n")
...
run_single <- function(cache_name, sst, chl, Groups, cache_dir,
                       n_years, dt_step, isave = 2) {
...
# Constant
cache_file <- file.path(cache_dir, "mdl_constant_300yr.rds")
if (!file.exists(cache_file)) {
  cat("Running: mdl_constant_300yr\n")
  env_constant <- createInputParams(time = seq(0, n_years, by = dt_step),
                                    sst = 15, chl = 1.0)
  mdl <- zoomss_model(input_params = env_constant, Groups = Groups,
                      isave = isave_freq)
  saveRDS(mdl, cache_file)
  cat("  Saved.\n")
} else {
  cat("Cached:  mdl_constant_300yr\n")
}

# Seasonal
cache_file <- file.path(cache_dir, "mdl_seasonal_300yr.rds")
if (!file.exists(cache_file)) {
  cat("Running: mdl_seasonal_300yr\n")
  env_seasonal <- createEnviroData(n_years = n_years, dt = dt_step, base_sst = 15,
  ...
  mdl <- zoomss_model(input_params = env_seasonal_params, Groups = Groups,
                      isave = isave_freq)
  saveRDS(mdl, cache_file)
  cat("  Saved.\n")
} else {
  cat("Cached:  mdl_seasonal_300yr\n")
}
...
n_to_run <- sum(!sapply(seq_len(nrow(task_grid)), function(row) {
  ...
  file.exists(file.path(cache_dir,
    paste0(sprintf("mdl_chl_log10_%.1f_sst%d_300yr", log10(chl_levels[i]), temp), ".rds")))
}))
...
if (n_to_run > 0) {
  ...
  clusterExport(cl, c("run_single", "task_grid", "chl_levels", "cache_dir",
                       "Groups", "n_years", "dt_step", "isave_freq"))
  ...
  results_chl <- parLapply(cl, seq_len(nrow(task_grid)), function(row) {
    ...
    run_single(cache_name, sst = temp, chl = chl_levels[i],
               ...
               n_years = n_years, dt_step = dt_step, isave = isave_freq)
  })
...
n_to_run <- sum(!sapply(sst_levels, function(temp) {
  file.exists(file.path(cache_dir, paste0(sprintf("mdl_sst%d_chl1_300yr", temp), ".rds")))
}))
...
if (n_to_run > 0) {
  ...
  clusterExport(cl, "pkg_path")
  clusterEvalQ(cl, devtools::load_all(pkg_path, quiet = TRUE))
  clusterExport(cl, c("run_single", "sst_levels", "cache_dir", "Groups",
                       "n_years", "dt_step", "isave_freq"))
